...
```{r}
unvotes |> 
  count(country, vote) |> #counts the number of votes for 'yes' 'no' and 'abstain' by each country
  group_by(country) |> #groups by country so all country votes are visible next to each other
  mutate(total = sum(n)) |> #adds the total number of votes per country regardless of voting decision 
  mutate(prop_vote = n/total) |> #calculates the proportion that each voting decision makes up from the total and creates a new column for it
  filter(country %in% c("United States", "Canada", 
                        "Germany", "France",
                        "Italy", "Japan",
                        "United Kingdom")) |> #filters by country so we only see United States, Canada, Germany, France, Italy, Japan, and the United Kingdom
  ggplot(aes(x = country, y = prop_vote, 
             fill = vote)) + #initiates a plot and provides data and fill color
  geom_col(position = position_stack()) + #intiates a column plot and alters the positon of the columns
  labs(x = "Group of Seven Countries",
       y = "Proportion of Votes",
       title = "Voting Record of the G7",
       fill = "Vote") + #labels important info like axes, titles, and what fill means
  theme_minimal() +  #changes the theme to a minimalistic theme
  scale_fill_viridis_d(end = 0.8) + #makes colors colorblindness-friendly
  coord_flip() #changes axes around visually, likely making the graph easier to interpret.
```

> The graph shows that most G7 countries do not abstain from UN votes, and that they abstain at a similar rate. Additionally, it's notable that the U.S. votes 'no' at a higher proportion than other countries. 
...
> Displays the 'short' column of the roll_calls data, which is essentially a short description of the topic being voted on.
...
> I think maybe a key describing the 5 most common issues would be helpful, along with a geom_point that may help illustrate the number of times the UN has voted on each topic.
...
> Not really, because each 'short' description is likely not alike, and so it counts each issue as an individual thing instead of grouping similar issues.
...
> They are fairly similar, but there are less rows in issues than in roll calls, indicating that the UN likely votes on the same issues multiple times.
...
```{r}
roll_calls |> 
  left_join(issues, by = "rcid") #joins issues to roll calls based on the rcid, keeping all the observations in roll calls
```

```{r}
roll_calls |> 
  right_join(issues, by = "rcid") #joins issues to rolls calls based off of rcid, keeping all values in issues Q: do i have my scenario x and y flipped?
```

```{r}
roll_calls |> 
  full_join(issues, by = "rcid") #joins issues and roll calls, keeping all observations in both.
```

```{r}
roll_calls |> 
  inner_join(issues, by = "rcid") #joins issues and roll calls, only keeping matching observations in both.
```
...
> I believe we would use an inner join because it only keeps matching observations from both data sets.
...
> I notice that the UN seems to have similar years of activity for each issue. This makes me wonder if the UN meets a majority of the time in wartimes to vote on conflicts.
...
```{r}
yes_votes |> 
  filter(country %in% c("United States","Canada")) |> # filtering data down to just US and Canada
  ggplot(mapping = aes(x = year, y = percent_yes, color = country)) + # initiated ggplot and determines mapping, x and y variables, and what color represents
  geom_point(alpha = 0.4) + # creates the geom point and also the geom line that I opted to have for clarity
  ...
  facet_wrap(~issue) + # facets visual based on the issue being voted on
  scale_y_continuous(labels = scales::percent) + #makes y continuous?
  labs(
  ...
  theme_bw() +
  theme(legend.position = "bottom") + #changes legend position for clarity 
  scale_color_viridis_d(option = "turbo") #makes colors friendly for colorblind people
```


What do you notice about the voting records over time? 

> The United States tends to have an overall lower percentage of yes votes on most issues than Canada.
